        ...
        if not src_data.rdd.isEmpty():
            ...
            if bool(table_dict.get('timestamp_cols')):
                delta_day_time = delta_day.select(
                    *(col(c).cast("timestamp").alias(c) if c.lower() in table_dict['timestamp_cols'].split(',') else c
                      for c in
                      delta_day.columns))
            else:
            ...
            if bool(table_dict.get('decimal_cols')):
                delta_day_final = delta_day_time.select(
                    *(col(c).cast("double").alias(c) if c.lower() in table_dict['decimal_cols'].split(',') else c for c
                      in
...
def sync_snapshot():
    try:
        ...
        merged_data.repartition(20).write.mode('overwrite').format('parquet').partitionBy(
            table_dict['partition_col_name']).save(target_path)
        logger.info('Write Final Data completed')

        os.system(''' aws s3 mv {0} s3://mbk-data-backup/snapshots_edits_backup/{1}/{2}/day={3}/ --recursive'''.format(
            source_path, table_dict['schema'], table_dict['table'], today_partition))
        # glueContext.purge_s3_path(source_path,{"retentionPeriod":0})
    except Exception as e:
        raise e
        logger.info(e)
